...
<warning>
This API endpoint has stricter limits in place to avoid a high volume of requests. This endpoint should be used only to check a domain's state before issuing a domain registration or a domain transfer. For other use case, we recommend using other services such as [DomainR](https://domainr.com/) 
</warning>
