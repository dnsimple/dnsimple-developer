...
You can decide your own sorting policy for each single API call via the `sort` param.
This param accepts a pair of informations: the name of a field and the order (`asc` for ascending and `desc` for descending).

This param accepts a set of comma separated key-value pairs: the name of a field and the order criteria (`asc` for ascending and `desc` for descending).
